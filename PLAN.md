...
| 2026-06-10 | テスト基盤 | 原因特定: ①テストターゲットがアプリに非依存(dependencies: [] — importすら不可能だった) ②このマシンはCLTのみでswift-testing/XCTestが無く`swift test`自体が実行不能。対応: 依存修正+PersistentVaultの実テスト5本(roundtrip/delete/cleanOrphans/no-op書き込みガード/200件性能)をswift-testing形式で追加、GitHub Actions CI(.github/workflows/ci.yml)を新設。**テストのコンパイル・実行はCIで初回検証される**(ローカル検証不能のため、push後にCI結果を要確認) |
