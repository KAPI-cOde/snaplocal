...
### [x] T9.18 ⌘⇧4パネルの「画像ズレ」解消+サイズのハイブリッド挙動(ユーザーFB 2026-06-13、方向性=ハイブリッドで承認)
- **背景FB**: 「切り抜き後ピタッと中央に出るのは気持ちいいが、アノテーションを選ぶとさらに画像がズレる(左右に余白)。拡大して注釈したい時に初期の狭いウィンドウが仇、でも最初から全画面だと軽快さが失われる」
- **診断(実測)**: ツール選択でツールバーにインライン調整(太さS/M/L等)が現れ、NSHostingView の sizingOptions=[.minSize] 経由でパネルの最小幅が増加 → ウィンドウが右に自動拡張(723→740(text)→758(redact)、ラチェット式で戻らない)。画像は実寸上限のため動かず左右余白が発生
- **承認済み方向(壁打ち選択肢3案からユーザー選択)**: **ハイブリッド** — ①ウィンドウは勝手に動かない ②手でパネルを広げた時だけ画像も追従拡大
- **実装**: ①パネル初期幅の下限を実測ツールバー最悪幅(758pt)+余裕=760ptへ → どのツール選択でも minSize が初期幅を超えず自動拡張が消滅 ②AnnotationCanvasView に fillsViewport フラグ(パネルのみ true)— autoFitZoom を 1.0(フィット基準・実寸上限なし)にし、既存の「!userZoomed ならビューポート追従」機構で手動リサイズに画像が追従
- **残論点**: 幅758未満の切り抜きは初期から左右余白が出る(ツールバー幅が物理下限)。気になるFBが来たらツールバーの段階的省略(狭幅時にインライン調整をポップオーバー化)を検討
- **受け入れ**: ①ツール選択・注釈選択でパネルのウィンドウ枠が動かない ②広い切り抜きで余白なし表示 ③パネルを手で広げると画像も大きくなる ④メインウィンドウの表示は従来どおり(実寸既定)

### [x] T9.17 矢印端点ドラッグの発見性・掴みやすさ(ユーザー要望 2026-06-12「始点と終点をドラッグで変更できると良い」)
- **重要な前提(起票時調査)**: **機能自体はT8.9で実装済み** — 矢印/直線を明示選択すると両端に白丸ハンドル(始点=グレー/終点=アクセント色)が出て、ドラッグで端点を個別移動できる(CanvasInteraction.beginHandleDragIfHit、インデックス9=start/10=end、shift角度制約対応)。ユーザーが「できない」と感じた=発見性または掴みやすさの問題の可能性が高い
- **診断の種**: ①端点ハンドルのヒット判定は±10ptの矩形 — T9.14で線本体の許容幅を22ptに広げた今、10ptはやや狭い(判定はハンドル先勝ちなので競合はしないが、外すと線グラブ=全体移動に化ける) ②**ミニアクションバー(T9.11)が選択枠上端に浮くため、上向き矢印では端点とバーが近接**し、バーのNSEventモニタがmouseDownを飲み込んで削除/複製に化ける事故が起こりうる(2026-06-12の合成E2Eで干渉を実測) ③描画直後はcreation選択でハンドル非武装(T8.9仕様)— 「描いてすぐ端点を直す」には一度選び直しが必要で、ここが体感の主因かも
- **ユーザー実機確認(2026-06-12)**: 「白丸をうまく掴めてないのか動かせない」— ハンドルの存在は認知済みでも掴めない=**発見性ではなく掴みやすさの実問題と確定**。着手時は ①端点判定の拡大(±10→14〜16pt、線本体22ptとの先勝ち関係は維持) ②ミニバーの端点回避配置 ③ズーム縮小時の実効判定幅の補正(T9.14で見送った論点と同根)の順で検討。修正後の検証は実機手動(合成ドラッグでは端点ハンドルを安定して掴めない — 進捗ログ参照)
- **受け入れ**: 端点を狙って動かせる・誤って削除/複製にならない。**真因判明(2026-06-13、ユーザー再FB「やはり掴めない」を受けた再診断)**: 掴みやすさではなく**端点ドラッグはT8.9実装当初から一度も機能していなかった**。①handleDragUpdateのJSON再ラップがCGPointを辞書`{x,y}`で書き込むが実際のエンコード形式は配列`[x,y]`のためdecodeが静かに失敗(完全no-op)→reverseArrow(T9.15)と同じ直接再構築へ変更 ②ドラッグ開始のヒット判定が最初のonChangedのlocation(速いドラッグで押下点から10〜40pt飛ぶ)→`value.startLocation`(押した点)で判定するよう修正。合成E2Eで数値検証済み: 高速端点ドラッグでstart不変・endがドラッグ量どおり+150pt移動・transform=identity。**ユーザー実機受け入れ待ち(白丸を狙って動かせるか)**

### [x] T9.16 目トグル(アノテーション表示切替)のツールバー撤去(ユーザーFB 2026-06-12)
- **背景**: 「目のアイコンがちょっと…。アノテーションの可視化無効化はツールバーになくてもいいかも」— 軽量UX方針(隠すはOK・消すはNG)に沿いツールバーから撤去し、到達経路は維持
- **実装**: ①ツールバーの目Toggleを削除 ②「その他(…)」メニューに Section「表示」>「アノテーションを隠す」を追加 ③アプリメニュー「表示」に同Toggle+⌘'(パネルもcanvas共有のため全ウィンドウで有効)
- **受け入れ**: ツールバーに目が無い・その他メニューと表示メニューから切替可・⌘'(JIS配列では⌘^に自動リマップ)で切替可
...
### [x] T9.7 OCRテキストの整形 — FoundationModelsで「軽く整える」(方針ユーザー承認 2026-06-12)
...
| 2026-06-12 | T9.16 | 目トグルをツールバーから撤去し「その他(…)」メニュー(Section表示)+アプリメニュー表示(Toggle+⌘')へ移設。パネルはstate.canvas共有のためメニュー1箇所で全ウィンドウ有効。実機E2E合格: ツールバーに目なし・その他メニューから非表示化(赤ピクセル0)・⌘'で再表示。**学び: ⌘'はJIS配列でOSが⌘^へ自動リマップ**(AXMenuItemCmdChar="^")— ツールバー内ラベルへのショートカット固定表記はやめ、表示メニューの自動表記に任せる。旧ツールバーToggleの.keyboardShortcut("'")はビューフォーカス依存で実は発火しづらかった(メニュー化でむしろ堅牢に) |
| 2026-06-12 | T9.15受け入れ+T9.17起票 | ユーザー「反転ok」でT9.15クローズ。新要望「矢印の始点と終点をドラッグで変更」→ 調査の結果**T8.9で実装済み**(明示選択→両端の白丸ハンドルをドラッグ)と判明、発見性/掴みやすさの問題としてT9.17起票(ユーザー了承で次回以降)。起票時調査で**ミニアクションバーが上向き矢印の端点ハンドルに近接しmouseDownを飲み込む干渉**を合成E2Eで実測 — 詳細はT9.17本文。合成端点ドラッグのE2Eは開始点1〜2step目がハンドル±10ptを外れやすくステップ数を増やしても安定せず、実機手動で確認するのが正 |
| 2026-06-12 | T9.17追記 | ユーザー実機FB「白丸をうまく掴めてないのか動かせない」— 実マウスでも掴めない=掴みやすさの実問題と確定(発見性ではない)。修正候補の優先順をT9.17本文に記載。次セッションの本命はT9.7(前提=Apple Intelligence有効化)、T9.17は判定拡大が小diffなので同セッション抱き合わせ可 |
| 2026-06-13 | T9.7 | Apple Intelligence有効化確認後、PoC実測→Codex実装+Fable 5レビュー。**PoC学び**: ①25字断片に455字の創作(比率18.2)→ minChars=200+文字数比0.7〜1.4の安全弁が必須と実証 ②モデルは「マークダウン禁止」指示を無視して**強調**や行頭`* `を足す→プロンプトでなく機械的strip(stripMarkdownDecoration)で除去 ③M1レイテンシ17〜44秒(2300字)。実装=承認スコープどおり(ocrTextPolished追加キー・二段書き・updateOCRでpolished無効化・updatePolishedOCRにstale guard・検索は生+polished・UI追加なし)。**Codexレビュー指摘2件**: PoC検証済みプロンプトの勝手な書き換えを復元/レガシーJSONテストにisStarred欠落(synthesized DecodableはBoolデフォルト値でもキー必須→CIで落ちる)を修正。実機E2E合格: デモvaultで回転→フォーク→生OCR(2292字)→約40秒後にpolished(2396字・比率1.05・MD混入なし)がJSONに永続化、再編集で生OCR変化→旧polished無効化→新raw用に再整形、を実測 |
| 2026-06-13 | T9.17 | 修正候補①を実施(Codex実装+Fable 5レビュー): 矢印/直線の端点ハンドル判定を±10→16ptへ拡大(CanvasInteraction.beginHandleDragIfHit、ハンドル先勝ちなので線本体22ptと競合しない)。吹き出し尻尾ハンドル(±10pt)は今回触らず — 同じ掴みづらさFBが出たら同手で拡大可。**ユーザー実機受け入れ残: 白丸を狙って掴める・ミニバーの削除/複製に化けない。**不十分なら②ミニバー端点回避→③ズーム補正で再起票 |
| 2026-06-13 | T9.17続報(真因) | ユーザー実機FB「±16ptでもやはり掴めない」→合成E2Eで無反応を再現し再診断。**端点ドラッグはT8.9から一度も動いていなかった**: handleDragUpdateのJSON再ラップがCGPointを辞書{x,y}で書くが実形式は配列[x,y]でdecodeが静かに失敗(try?が握りつぶし)。直接再構築(reverseArrow方式)へ変更+ヒット判定をvalue.startLocation(押下点)へ修正(最初のonChangedは速いドラッグで10〜40pt飛ぶ — 「合成ドラッグでは端点を掴めない」という従来の制約もこれが原因で、修正後は合成E2Eが安定して通る)。数値E2E合格: start[462.5,293]不変・end[662.5,293]→[662.5,443](+150=ドラッグ量どおり)。**学び: try?でJSON再ラップする変形は形式不一致が無音になる — 注釈の点変更は直接再構築が正** |
| 2026-06-13 | T9.18 | ユーザーFB(パネルの画像ズレ+サイズの気持ちいいバランス)を壁打ち3案からハイブリッドで確定し実装(Codex実装+Fable 5レビュー)。根因実測: ツール選択→インライン調整出現→NSHostingView sizingOptions=[.minSize]がパネル最小幅を増やしウィンドウが右へラチェット拡張(723→758)。修正: ①初期幅下限=全17ツール実測の最悪幅758+余裕=760 ②fillsViewportフラグ(パネルのみ)でautoFitZoom=1.0(フィット基準・実寸上限なし)— 既存の「!userZoomedならビューポート追従」がそのままハイブリッドの核に。合成E2E合格: 17ツール一巡でパネル枠不変(340,274,760,382)・AXで1100×550へ手動リサイズ→ズームバッジ157%=フィット比どおり画像追従(赤px高さ×1.47/数×2.17≒k²)。**学び: ウィンドウ最小幅の実測はAXでset sizeして戻り値を見る(下限にクランプされる)/sizingOptions=[.minSize]はコンテンツ膨張でウィンドウを勝手に広げるラチェット** |
| 2026-06-13 | T9.17+T9.18受け入れ | ユーザー実機確認「バッチリ治っていました」— 端点ドラッグ(T9.17根治)とパネルのハイブリッド挙動(T9.18)を受け入れOKでクローズ。T9.18の残論点(幅758未満の切り抜きは初期から左右余白)は実使用FB待ちのまま |
