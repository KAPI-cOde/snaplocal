...
| 2026-06-12 | T9.15 FB修正 | ユーザーFB「向きを反転が出ない」— 根因: 注釈系メニューは選択中のみ表示で、**右クリックは選択しない**ため未選択の矢印では出なかった。修正: RightClickSelectionHandler(CanvasHelpers)= rightMouseDown/ctrl+左クリックのNSEventローカルモニタ(T9.11方式)でメニューが開く前にカーソル直下の注釈を自動選択(イベントは飲まない・hitTest=nilで他操作に透過・isFlipped=trueでSwiftUI座標)。toCanvas漏斗で座標変換、annotationsHidden/cropモード/テキスト入力中はスキップ。実機E2E合格: 未選択矢印を右クリック→自動選択+「向きを反転」表示→クリックで矢じりが反対端へ(ピクセル走査で確認)→⌘Zで復帰→再起動後も注釈保持。左ドラッグ描画・クリック選択・ミニアクションの回帰なし。E2Eノウハウ: 矢じりの反転検証は「両端±40ptの赤ピクセル数比較」が有効だが**選択ハンドル(白丸)が矢じり先端を覆い数値が激減する**ため最終判定はクロップ画像の目視で |
