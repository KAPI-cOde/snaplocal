...
### [ ] T8.3 導線整理(設定・ヘルプ・旧挙動の整合)
- openEditorOnCapture / autoCopyOnCapture と新フローの整合(新フロー=既定。フルエディタ直行はオプションとして残す)
- ⌘⌃4(クリップボードのみ)との関係整理、ヘルプ・メニュー文言更新
...
| 2026-06-11 | T8.2 | Fable 5直接実装(canvasSize所有権領域のため)。QuickAnnotatePanel.swift新設(借りないborderless NSPanel、canBecomeKey、Esc=cancelOperation、画面70%クランプ+中央表示)。**canvasSize単一書き込み者の保証**: パネル表示中は ContentView が `quickPanelActive` でメイン側 AnnotationCanvasView をヒエラルキーから外し、閉じると再挿入の onAppear が canvasSize を取り戻す設計。完了(⌘↩/ボタン)=copyToClipboard(注釈込み)+updateAnnotations即時永続化+閉じ、Esc=保持して閉じるのみ。ペースト/ドロップは suppressQuickPanel(clipboardOnlyCaptureと同作法)でパネル抑止しHUD維持。vault保存/OCR/QRはacceptCapture内Taskで不変。ビルド通過+起動/⌘⇧4/ESCスモーク。気づき(T8.3送り): パネル内ツールバーからのペーストでHUDが併出/サイドバートグルがパネル内で無意味/完了時のstatusChipがメイン非表示中で見えない |
