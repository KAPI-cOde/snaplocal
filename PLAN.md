...
### [ ] T9.3 クイック注釈パネルの移動・サイズ変更(ユーザー要望 2026-06-12)
...
### [ ] T9.4 顔検出(detectFaceRects)のスレッド違反クラッシュ修正(クラッシュレポート 2026-06-11 21:26)
...
- **方針の種**: 完了ハンドラ内では値の取り出しのみ行い、MainActor への反映は `Task { @MainActor in ... }` 等で隔離境界を明示的に越える。reRunOCR / detectBarcodes 等の同型コードも横並び点検
- **受け入れ**: 着手時に定義(顔検出の実行経路で落ちないこと)

### [ ] T9.5 アノテーション座標のウィンドウサイズ非依存化(ユーザーFB 2026-06-12「エディタで開いた後、最大化されていないとアノテーションの位置が正しくない」)
...
- **方針の種**: ①fit変化時に注釈へ scale(newFit/oldFit) を適用するライブ追従 ②保存時に基準canvasSizeを新キー(追加のみ・互換OK)で記録し、ロード時に現fitへ再スケール。旧データ(キーなし)は現挙動のまま。canvasSize所有権(CLAUDE.md最重要落とし穴)と書き出し(renderAnnotations のscale計算)への影響を要設計レビュー — **着手前に設計をユーザーへ提示**
- **受け入れ**: パネルで注釈→エディタを任意サイズで開いても位置一致。ウィンドウリサイズ中も注釈が画像に追従。既存vaultの表示が悪化しない

### [ ] T9.6 履歴の概要表示の再設計(ユーザーFB 2026-06-12「吹き出しの体験いまいち。メモも入れられない」)
- **背景**: T9.1でホバーポップオーバーを固定化したが、ユーザー評価は「クリックすれば読めるがホバーだけだと消える/体験がいまいち/メモが入れにくい」。ホバー依存の表示そのものに限界
- **論点の種**: 選択アイテムの詳細表示を「ホバーで出る吹き出し」から「明示操作で出る固定面」へ — 候補: ①サイドバー下部の詳細ペイン(選択中アイテムのOCR/メモ常設) ②QuickLook(Space)へのOCR/メモ統合 ③吹き出しは廃止または画像プレビューのみに縮小。軽量UX原則(表面コントロール最小)との両立を設計してから提案・承認
- **受け入れ**: 提案時に定義

### [ ] T9.7 OCRテキストの整形(ユーザー質問 2026-06-12「OCR後の文章がメチャクチャ。Appleシリコンで整形できないか」)
- **背景**: Vision OCRは複数カラム・UIレイアウトのスクリーンショットで読み順が乱れ、検索には使えるが「読む」品質ではない
